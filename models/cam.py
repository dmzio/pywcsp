...
class Camera(object):
    ...
    def switch_autoexposure(self):
        ...
        cur_val = check_output('uvcdynctrl -g "Exposure, Auto"', stderr=subprocess.STDOUT, shell=True)
        if int(cur_val) == 3:
            new_val = 1
        else:
            new_val = 3

        check_call('uvcdynctrl -s "Exposure, Auto" {0}'.format(new_val), shell=True)

        return check_output('uvcdynctrl -g "Exposure, Auto"', stderr=subprocess.STDOUT, shell=True)

    def get_exposure_value(self):
        return check_output('uvcdynctrl -g "Exposure (Absolute)"', stderr=subprocess.STDOUT, shell=True)
